...
function YoutubeMp3Downloader(options) {
    ...
    self.downloadQueue = async.queue(function (task, callback) {
        ...
        var videoId = task.videoId;
        var songTitle = task.songTitle;
        var songArtist = task.songArtist;

        self.performDownload(videoId, songTitle, songArtist, function(err, result) {
            callback(err, result);
        });
...
YoutubeMp3Downloader.prototype.download = function(videoId, songTitle, songArtist) {

    var self = this;

    self.downloadQueue.push({videoId: videoId, songTitle: songTitle, songArtist: songArtist}, function (err, data) {
        if (err) {
            self.emit("error", err);
        } else {
...
YoutubeMp3Downloader.prototype.performDownload = function(videoId, songTitle, songArtist, cb) {
    ...
    ytdl.getInfo(videoUrl, function(err, info){
        ...
        var fileName = self.outputPath + '/' + songTitle + '.mp3';
